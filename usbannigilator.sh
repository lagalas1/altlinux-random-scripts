...
while true; do
  ...
  if [ "$PCS" == "" ]; then
    ...
    for PC in $PCS; do
      ...
      if $(sshpass -p "$PASSW" scp -o "StrictHostKeyChecking=accept-new" -r ForRemote/* "$SSHUSER"@"$PC":/home/OGVKK.RU/d_a_iashchenko/); then
